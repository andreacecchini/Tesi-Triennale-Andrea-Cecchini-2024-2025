...
\acrodef{AP}{Programmazione Aggregata}
...
La \ac{AP} \cite{AC} è un paradigma di macroprogrammazione \cite{macroprogramming} ideato per lo sviluppo di Sistemi Collettivi Adattivi (CAS). 
%
...
\ac{AP} semplifica la complessità della coordinazione distribuita grazie a una struttura a più livelli di astrazione (\cref{fig:ac-architecture}), costruita a partire dal field calculus \cite{AC}. Questa architettura consente di colmare il divario di astrazione, offrendo agli sviluppatori un'API semplice e intuitiva.

La scelta di basarsi su un calcolo minimale e matematico permette di dimostrare formalmente le proprietà dei costrutti fondamentali e di definire operatori generali con caratteristiche verificabili. Questi operatori possono essere utilizzati per realizzare librerie di alto livello, garantendo che le proprietà valide per i singoli operatori si mantengano anche nelle loro composizioni.
\begin{figure}
    \centering
...
Il \textit{Field Calculus} (FC) è un linguaggio funzionale minimale, progettato per offrire i costrutti fondamentali necessari alla manipolazione dei campi \cite{HFC}. In questo formalismo, ogni espressione rappresenta un \textit{campo computazionale}, che costituisce l’astrazione unificante del linguaggio.
...
%
In ogni round, il dispositivo i) raccoglie i valori dal proprio campo di vicinato \(\phi_i\); ii) ottiene le informazioni dall'ambiente (eg. sensori); iii) recupera lo stato interno memorizzato nell’ultimo round, se disponibile; (iv) valuta il programma \texttt{P} utilizzando queste informazioni; v) aggiorna il proprio stato e invia il valore risultante ai vicini; vi) attende il successivo round di esecuzione \cite{HFC}.
...
\begin{itemize}
    ...
    \item Dinamica Temporale: $\texttt{rep(e0) \{(x) => e\}}$ è fondamentale per modellare l'evoluzione nel tempo di un campo computazionale. Inizializza una variabile di stato locale $\texttt{x}$ con il valore iniziale $\texttt{e}_0$ e, a ogni round di esecuzione, aggiorna $\texttt{x}$ con il risultato dell'espressione $\texttt{e}$. L'espressione $\texttt{e}$ può fare riferimento al valore precedente di $\texttt{x}$, consentendo la definizione di processi iterativi.
    ...
    \item Restrizione dello spazio: \texttt{if (\(e_0\)) \{\(e_1\)\} else \{\(e_2\)\}} partiziona lo spazio dei dispositivi in due sottoinsiemi \(D_{true}\) e \(D_{false}\) in base alla valutazione di \texttt{\(e_0\)}. Se un dispositivo \(\delta\) appartiene a \(D_{true}\), valuta esclusivamente l’espressione \(e_1\), ignorando \(e_2\); viceversa, se appartiene a \(D_{false}\), valuta solo \(e_2\). In entrambi i casi, le informazioni di stato e i valori scambiati con i vicini non vengono condivisi tra i due rami: quando un dispositivo passa da un ramo all’altro, le variabili di stato vengono reinizializzate e i valori precedenti vengono persi. Se è necessario condividere informazioni tra i rami, si può utilizzare il costrutto \texttt{mux(\(e_0, e_1, e_2\))}, che valuta entrambe le espressioni \texttt{\(e_1\)} e \texttt{\(e_2\)}, senza partizionare lo spazio, restituendo il valore corrispondente a \texttt{\(e_0\)}.
    ...
    \]
    Nonostante la sintassi sia identica a quella di \texttt{rep}, i due si distinguono radicalmente per l'interpretazione della variabile \texttt{x} ad ogni round. Nel caso di \texttt{share}, \texttt{x} non rappresenta un valore locale (come in \texttt{rep}), bensì un campo di vicinato ($\phi$) che contiene i valori condivisi dai vicini nel round precedente. L'espressione \texttt{\(e_2\)} ha quindi il compito di elaborare questo campo di vicinato per produrre il nuovo valore locale, che sarà poi utilizzato come \texttt{\(e_1\)} nel round successivo e condiviso con i vicini.
\end{itemize}

\paragraph{Allineamento} Ogni dispositivo deve comunicare esclusivamente con i dispositivi che si trovano nello stesso contesto di esecuzione, ossia che hanno seguito lo stesso percorso di valutazione del programma: hanno invocato le stesse funzioni e preso le stesse decisioni. Tali dispositivi si definiscono \textit{allineati}.

Si consideri l'esempio in \cref{lst:alignment}, in cui sono definite due funzioni, \texttt{f1} e \texttt{f2}, entrambe contenenti il costrutto \texttt{nbr} applicato alla stessa espressione \texttt{e1}. Quando un dispositivo esegue \texttt{nbr \{e1\}} all'interno di \texttt{f1}, deve recuperare i valori dei vicini che hanno valutato quella stessa espressione nello stesso contesto, ovvero all'interno di \texttt{f1}. Se invece un vicino ha calcolato \texttt{nbr \{e1\}} all'interno di \texttt{f2}, il suo valore non è rilevante per il dispositivo che sta eseguendo \texttt{nbr \{e1\}} in \texttt{f1}.
...
Questo aspetto risulta particolarmente evidente nel costrutto \texttt{if}, che non si limita a determinare quale ramo eseguire, ma suddivide lo spazio dei dispositivi in due gruppi distinti, ciascuno con il proprio contesto di esecuzione. In questi casi, l’allineamento diventa fondamentale per garantire che la comunicazione avvenga solo tra dispositivi che hanno scelto lo stesso ramo.

Poiché i nodi non possiedono una memoria condivisa per tenere traccia dello stato della computazione su ciascuno, è necessario introdurre meccanismi di allineamento che permettano di monitorare lo stato computazionale di ogni dispositivo e di individuare quali sono effettivamente allineati tra loro. In un linguaggio di programmazione ideale, questi dettagli di basso livello dovrebbero rimanere trasparenti all’utente finale.

\paragraph{High-Order Field Calculus} \ac{FC} è stato esteso in \cite{HFC} con l’introduzione dell'\ac{HFC}, che consente di trattare le funzioni come cittadini di prima classe, ovvero come qualsiasi altro valore. Questa estensione abilita alcune capacità fondamentali: i) le funzioni possono accettare altre funzioni come argomenti e restituirle come risultati; ii) possono essere create dinamicamente; iii) possono essere trasferite tra dispositivi (tramite \texttt{nbr}) e memorizzate o modificate nel tempo (tramite \texttt{rep}); iv) inoltre, durante un partizionamento spaziale, i dispositivi possono eseguire funzioni diverse in base alla valutazione di una condizione.

\subsection{XC Calculus}
\label{sec:xc}

\ac{XC} è un linguaggio di programmazione funzionale sperimentale che generalizza ed estende le capacità di \ac{FC} \cite{XC}, formalizzando un insieme di meccanismi chiave, ancora più minimali rispetto a \ac{FC}, per esprimere in modo essenziale il comportamento complessivo dei sistemi collettivi distribuiti.
...
Questa unificazione semplifica notevolmente il sistema di tipi rispetto a \ac{FC} e, grazie alla flessibilità degli \texttt{nvalues}, \ac{XC} risulta più espressivo, consentendo a ciascun dispositivo di inviare messaggi diversi a vicini differenti (comunicazione \textit{anisotropica}).
...
Proto è caratterizzato come un linguaggio puramente funzionale con una sintassi ispirata al Lisp (\Cref{lst:mitprotoexample}).
...
Lo sviluppo del linguaggio è stato interrotto nel 2016 e il linguaggio è stato ufficialmente dismesso in favore di Protelis \cite{protelis}  (\Cref{sec:protelis}), che ne ha raccolto l'eredità concettuale e operativa.
...
\emph{Protelis} \cite{protelis} è un linguaggio di programmazione funzionale noto per essere il primo ad aver implementato i concetti del \ac{HFC} (\Cref{sec:fieldcalculus}). La sua sintassi è ispirata al linguaggio C, il che lo rende più accessibile rispetto al suo predecessore, Proto (\Cref{sec:mitproto}), ed è caratterizzato dall'essere debolmente tipizzato (\Cref{lst:protelisexample}).
...
Proto è un linguaggio puramente funzionale caratterizzato dall'utilizzo di \ac{sexp} (\ref{lst:mitprotosexp}),  ovvero espressioni in notazione polacca, seguendo una sintassi molto simile a quella di Scheme\footnote{\url{https://www.scheme.org/}}.
...
Il linguaggio, progettato per essere multipiattaforma grazie all’adesione al progetto \ac{KTM}\footnote{\url{https://kotlinlang.org/docs/multiplatform.html}}, permette di eseguire il codice su diverse piattaforme (ad esempio JVM, JavaScript e nativo) con modifiche minime. L’intero processo di build è gestito tramite Gradle\footnote{\url{https://gradle.com/}}.
