...
Il panorama della programmazione aggregata è caratterizzato da diversi linguaggi e framework che implementano i concetti fondamentali del field calculus, ognuno con specifici vantaggi e svantaggi. 
...
È un linguaggio puramente funzionale, con una sintassi ispirata al Lisp (\Cref{lst:mitprotoexample}).
\lstinputlisting[
    language=Lisp,
...
\emph{Protelis} \cite{protelis} è un linguaggio di programmazione funzionale ed è il primo noto ad aver implementato i concetti dell'High-Order Field Calculus (\Cref{sec:fieldcalculus}). La sua sintassi è ispirata al linguaggio $\text{C}$, il che lo rende più accessibile rispetto al suo predecessore Proto (\Cref{sec:mitproto}). È inoltre caratterizzato dall'essere debolmente tipizzato (\Cref{lst:protelisexample}).

$\text{Protelis}$ è un linguaggio stand-alone basato sulla Java Virtual Machine (JVM). Sfrutta $\text{Xtext}$, un framework per la creazione di \ac{DSL}, che fornisce strumenti e librerie essenziali per lo sviluppo.
%
L'esecuzione dei programmi avviene tramite un interprete che valuta periodicamente il codice pre-parsato. Questo gestisce in modo automatico la comunicazione tra dispositivi e l'interazione con l'ambiente. Poiché l'interprete è stato scritto da zero, i meccanismi di basso livello, come l'allineamento, sono gestiti in modo trasparente per l'utente finale.

L'integrazione con la JVM garantisce portabilità tra sistemi e dispositivi, oltre a facilitare l'importazione di numerose librerie e API tramite i meccanismi di reflection di Java. Tuttavia, la necessità di una Java Virtual Machine rappresenta una limitazione per l'impiego in ambienti con risorse estremamente ristrette, come quelli tipici dell'\ac{IoT}, riducendo l'eterogeneità dei dispositivi utilizzabili.
...
Offre un ecosistema completo per lo sviluppo di programmi aggregati, comprendente librerie, strumenti di simulazione e la possibilità di prototipazione rapida anche via browser grazie a ScaFi Web \cite{scafiweb}.
...
Questa scelta, tuttavia, comporta anche alcune limitazioni: la sintassi è vincolata ai costrutti validi di Scala e, aspetto cruciale per la programmazione aggregata, possono emergere involontariamente dettagli di basso livello che non dovrebbero essere esposti all’utente finale, come la gestione dell’allineamento. Inoltre, essendo pensato principalmente per l’esecuzione sulla Java Virtual Machine, ScaFi condivide le stesse limitazioni di applicabilità già riscontrate in Protelis (\ref{sec:protelis}).
...
\emph{FCPP} \cite{fcpp} rappresenta la prima implementazione nativa della programmazione aggregata in \(\text{C++}\).

L’utilizzo di \(\text{C++}\) come linguaggio di base è un elemento chiave, poiché consente l’esecuzione diretta del codice sui dispositivi, senza la necessità di una macchina virtuale. Questo approccio offre due vantaggi principali: da un lato, risulta particolarmente adatto a dispositivi con risorse limitate, tipici dell'\ac{IoT}, superando così le restrizioni imposte dalla dipendenza dalla \ac{JVM} che caratterizzano soluzioni come Protelis e ScaFi; dall’altro, permette di ottenere prestazioni elevate nell’implementazione del paradigma aggregato.
...
Questo lavoro si propone di trasporre una serie di esempi di programmazione aggregata realizzati originariamente in Proto (\Cref{sec:mitproto}) al linguaggio Collektive (\Cref{chap:trasposizione}).
